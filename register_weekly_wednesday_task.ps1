...
<#
.SYNOPSIS
    Register the JRA-VAN weekly Wednesday sync job in Windows Task Scheduler.
.DESCRIPTION
    Registers a task that runs weekly_wednesday_sync.ps1 every Wednesday at 23:00.
...
.PARAMETER TaskName
    Task name (default: JLTSQL_WeeklyWednesdaySync)
.PARAMETER Force
    Overwrite an existing task with the same name.
.EXAMPLE
    .\register_weekly_wednesday_task.ps1 -PgPassword "kousuke0809"
    .\register_weekly_wednesday_task.ps1 -Force
.NOTES
    - The task runs as the currently logged-in user.
    - The PC must be on and logged in on Wednesday at 23:00.
    - StartWhenAvailable is enabled, so missed runs are executed on next boot/login.
#>

param(
    [string]$PgPassword = "",
    [string]$TaskName   = "JLTSQL_WeeklyWednesdaySync",
    [switch]$Force
)
...
$syncScript = Join-Path $root "weekly_wednesday_sync.ps1"

if (-not (Test-Path $syncScript)) {
    throw "weekly_wednesday_sync.ps1 not found: $syncScript"
}
...
PostgreSQL password is required. Specify it using one of:
  -PgPassword argument:
    .\register_weekly_wednesday_task.ps1 -PgPassword "yourpassword"
  PGPASSWORD environment variable:
    `$env:PGPASSWORD = "yourpassword"
    .\register_weekly_wednesday_task.ps1
"@
}
...
if ($existingTask) {
    if ($Force) {
        ...
        Write-Host "  .\register_weekly_wednesday_task.ps1 -Force"
        Write-Host ""
...
$trigger = New-ScheduledTaskTrigger -Weekly -DaysOfWeek Wednesday -At "23:00"
...
Register-ScheduledTask `
    ...
    -Settings    $settings `
    -RunLevel    Highest `
    -Description "JRA-VAN weekly Wednesday sync: past results + index pipeline (localhost:5432/keiba)" | Out-Null
